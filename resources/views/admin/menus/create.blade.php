...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-headline font-bold text-gray-900 tracking-tight">Racik Menu Baru</h1>
    <p class="text-gray-500 text-sm mt-2 max-w-2xl leading-relaxed">
        Buat menu baru dan tentukan komposisi bahan baku (BOM). Sistem akan menghitung HPP secara otomatis berdasarkan data gudang.
    </p>
</div>
            ...
            <div class="bg-white p-8 rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.03)] border border-gray-100">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="number" name="selling_price" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm font-bold text-primary focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Deskripsi Singkat</label>
                        <textarea name="description" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"></textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.03)] border border-gray-100 overflow-hidden">
                <div class="p-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Estimasi HPP akan dikalkulasi otomatis.</p>
                    </div>
...
<script>
    ...
    function addRow() {
        ...
        materials.forEach(m => {
            options += `<option value="${m.id}" data-price="${m.moving_average_price}">${m.name} (${m.unit})</option>`;
        });
        ...
        const html = `
            <tr id="row-${rowIdx}" class="hover:bg-gray-50/50 transition-colors">
                ...
                <td class="p-5">
                    <input type="number" step="0.01" name="items[${rowIdx}][qty]" class="qty-input w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" oninput="calculateRow(${rowIdx})" required>
                </td>
                <td class="p-5">
